...
# Script de sauvegarde unifié Fast Foodie
...
# Fonction d'aide
show_help() {
    echo "🔄 Script de sauvegarde unifié Fast Foodie"
    echo ""
...
log "🚀 Début de la sauvegarde unifiée Fast Foodie..."
...
DB_COUNT=$(ls -1 "${DB_BACKUP_DIR}"/*.sql 2>/dev/null | wc -l)
IMAGES_COUNT=$(ls -1 "${IMAGES_BACKUP_DIR}"/images_backup_*.tar.gz 2>/dev/null | wc -l)

log "   📁 Base de données: ${DB_COUNT} sauvegardes (${TOTAL_DB_SIZE})"
log "   🖼️  Images: ${IMAGES_COUNT} sauvegardes (${TOTAL_IMAGES_SIZE})"

log "🎉 Sauvegarde unifiée terminée avec succès!" 